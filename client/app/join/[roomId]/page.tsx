...
function GuestScreenContent() {
  ...
  async function handleSkip() {
    if (!roomCode || !nowPlaying?.videoId || skipping) return;
    setSkipping(true);
    try {
      ...
      setAddedMessage(
        err instanceof Error ? err.message : "Failed to skip song"
      );
      setTimeout(() => setAddedMessage(null), 2500);
    } finally {
      setSkipping(false);
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            {nowPlaying?.videoId && (
              <button
                ...
                onClick={handleSkip}
                disabled={skipping}
                className="ktv-btn-secondary shrink-0 rounded-xl px-3 py-2 text-xs font-semibold text-white disabled:opacity-50 sm:px-4 sm:py-2.5 sm:text-sm"
              >
